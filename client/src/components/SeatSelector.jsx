...
const SeatSelector = ({ onSeatsChange, showtimeId, movieTitle, refreshTrigger }) => {
  ...
  const fetchOccupiedSeats = useCallback(async () => {
    try {
      ...
      const url = `http://localhost:5000/api/bookings/occupied-seats?showtime_id=${effectiveShowtimeId}&movie_title=${encodedMovieTitle}&_=${Date.now()}`;
  ...
  useEffect(() => {
    const ws = new WebSocket(`ws://localhost:5000/ws?showtime=${showtimeId}`);
